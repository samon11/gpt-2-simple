...
class HParams():
    def __init__(self, n_vocab, n_ctx, n_embd, n_head, n_layer, dtype):
...
def norm(x, scope, *, axis=-1, epsilon=1e-5, dtype=default_hparams().dtype):
    """Normalize to mean = 0, std = 1, then do a diagonal affine transform."""
    with tf.compat.v1.variable_scope(scope):
        n_state = x.shape[-1].value
        g = tf.compat.v1.get_variable('g', [n_state], initializer=tf.compat.v1.constant_initializer(1), dtype=dtype)
        b = tf.compat.v1.get_variable('b', [n_state], initializer=tf.compat.v1.constant_initializer(0), dtype=dtype)
...
def conv1d(x, scope, nf, *, w_init_stdev=0.02, dtype=default_hparams().dtype):
    with tf.compat.v1.variable_scope(scope):
        *start, nx = shape_list(x)
        w = tf.compat.v1.get_variable('w', [1, nx, nf], initializer=tf.compat.v1.random_normal_initializer(stddev=w_init_stdev), dtype=dtype)
        b = tf.compat.v1.get_variable('b', [nf], initializer=tf.compat.v1.constant_initializer(0), dtype=dtype)
...
def attn(x, scope, n_state, *, past, hparams):
    ...
    with tf.compat.v1.variable_scope(scope):
        c = conv1d(x, 'c_attn', n_state*3, dtype=hparams.dtype)
        ...
        a = merge_heads(a)
        a = conv1d(a, 'c_proj', n_state, dtype=hparams.dtype)
        return a, present
...
def mlp(x, scope, n_state, *, hparams):
    with tf.compat.v1.variable_scope(scope):
        nx = x.shape[-1].value
        h = gelu(conv1d(x, 'c_fc', n_state, dtype=hparams.dtype))
        h2 = conv1d(h, 'c_proj', nx, dtype=hparams.dtype)
        return h2
...
def block(x, scope, *, past, hparams):
    with tf.compat.v1.variable_scope(scope):
        nx = x.shape[-1].value
        a, present = attn(norm(x, 'ln_1', dtype=hparams.dtype), 'attn', nx, past=past, hparams=hparams)
        x = x + a
        m = mlp(norm(x, 'ln_2', dtype=hparams.dtype), 'mlp', nx*4, hparams=hparams)
        x = x + m
        return x, present
...
def model(hparams, X, past=None, scope='model', reuse=False):
    with tf.compat.v1.variable_scope(scope, reuse=reuse):
        ...
        h = norm(h, 'ln_f', dtype=hparams.dtype)
